...
class PortScanner(ScanModule):
    ...
    async def quick_scan(self) -> PortScanModel:
        ...
        live_hosts = await ping_sweep(self.target, self.verbose, self.progress_callback, unprivileged=self.unprivileged)
        if not live_hosts:
        ...
        if self.verbose:
            with utils.get_progress() as progress:
                probe_xml, _ = await _probe_services(
                    host_ports, self.verbose, self.progress_callback, progress, unprivileged=self.unprivileged
                )
        else:
            probe_xml, _ = await _probe_services(host_ports, self.verbose, self.progress_callback, unprivileged=self.unprivileged)
        hosts_by_ip = {}
...
async def quick_scan(
    ...
    Returns:
        PortScanModel with scan results.
    """
    return await PortScanner(target, verbose, progress_callback, unprivileged=unprivileged).quick_scan()
...
async def full_scan(
    ...
    Returns:
        PortScanModel with scan results.
    """
    return await PortScanner(target, verbose, progress_callback, unprivileged=unprivileged).full_scan()
